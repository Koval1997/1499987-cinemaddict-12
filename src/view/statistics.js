    ...
    if (evt.target.classList.contains(`statistic__filters-input`)) {
      ...
      switch (evt.target.value) {
        ...
        case StatisticPeriods.TODAY:
          update = {
            watchedFilms: this._films.filter((film) => film.isWatched && moment(film.watchingDate).isSame(new Date(), `day`)),
            currentPeriod: StatisticPeriods.TODAY
          };
          break;
        case StatisticPeriods.WEEK:
          update = {
            watchedFilms: this._films.filter((film) => film.isWatched && moment(film.watchingDate).isBetween(dateAWeekAgo, new Date())),
            currentPeriod: StatisticPeriods.WEEK
          };
          break;
        case StatisticPeriods.MONTH:
          update = {
            watchedFilms: this._films.filter((film) => film.isWatched && moment(film.watchingDate).isBetween(dateAMonthAgo, new Date())),
            currentPeriod: StatisticPeriods.MONTH
          };
          break;
        case StatisticPeriods.YEAR:
          update = {
            watchedFilms: this._films.filter((film) => film.isWatched && moment(film.watchingDate).isBetween(dateAYearAgo, new Date())),
